...
**SortedSet（interface）**：是TreeSet的父类，可排序集合，元素可自动按照大小顺序排序
...
LinkedList实现了List[接口](http://baike.baidu.com/view/159864.htm)，允许null元素。此外LinkedList提供额外的get，remove，insert方法在LinkedList的首部或尾部。这些操作使LinkedList可被用作堆栈（stack），队列（queue）或双向队列（deque）。
...
HashMap集合key部分的元素，以及HashSet集合中的元素，需要同事重写hashCode和equals方法。
...
对于经常改变的数据，可以写道文件中，使用程序动态获取。将来只需要修改这个文件就可以了，java代码不需要改动，也不需要重新编译，服务器也不需要重新启动，就可以拿到动态的信息了。这就是属性配置文件
...
泛型这种机制，只在程序编译极端起作用，只是给编译器参考的（运行阶段没用）
